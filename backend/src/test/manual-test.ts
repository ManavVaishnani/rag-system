...
async function runManualTests() {
  ...
  try {
    ...
    try {
      ...
      logger.error('❌ Qdrant connection failed. Check QDRANT_URL and QDRANT_API_KEY');
      logger.error(error);
    }
    ...
    try {
      ...
      logger.info(`✅ Gemini Embedding successful (Dimensions: ${embedding.length})`);
    } catch (error) {
    ...
    try {
      ...
      const response = await llmService.generateResponse('Say "RAG System Online"', []);
