...
describe('Document Process Flow', () => {
  ...
  describe('processDocument', () => {
    ...
    it('should successfully parse, chunk, embed, and store document data', async () => {
      // 1. Mock Parse result
      const parsedText = 'This is a test document content. It has multiple sentences to test chunking flow.';
      mockParser.parseDocument.mockResolvedValue(parsedText);

      // 2. Mock Chunking result
      const mockChunks = [
        { content: 'This is a test document content.', index: 0, metadata: { startChar: 0, endChar: 31 } },
        { content: 'It has multiple sentences to test chunking flow.', index: 1, metadata: { startChar: 32, endChar: 80 } }
      ];
      ...
      (prisma.documentChunk.createMany as jest.Mock).mockResolvedValue({ count: 2 });
      (prisma.document.update as jest.Mock).mockResolvedValue({ id: documentId });

      // Execute the private method using array notation to bypass access modifier
      await (documentController as any).processDocument(documentId, filePath, mimeType, userId, filename);
      ...
      expect(mockEmbedding.batchGenerateEmbeddings).toHaveBeenCalledWith(mockChunks.map(c => c.content));
    ...
    it('should fail and update status if any service throws an error', async () => {
      ...
      mockChunking.chunkText.mockReturnValue([{ content: 'Chunk 1', index: 0, metadata: { startChar: 0, endChar: 7 } }]);
    ...
    it('should handle unsupported file types by throwing error in parser', async () => {
      ...
      mockParser.parseDocument.mockRejectedValue(new Error(`Unsupported file type: ${unsupportedMime}`));

      await (documentController as any).processDocument(documentId, filePath, unsupportedMime, userId, filename);
