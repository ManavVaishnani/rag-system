...
async function createCollection() {
  ...
  console.log(`Checking collection '${collectionName}' with vector size ${vectorSize}...`);
  ...
  try {
    ...
    if (exists) {
      ...
      // @ts-ignore
      const size = info.config?.params?.vectors?.size || info.config?.params?.vectors?.['']?.size || info.config?.params?.vectors;
      
      console.log(`Existing vector size: ${size}`);
      
      if (size !== vectorSize) {
        console.warn(`WARNING: Existing size ${size} matches config ${vectorSize}.`);
